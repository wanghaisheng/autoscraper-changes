...
async def downloadpdf():
    ...
    if len(data.data) > 0:
        ...
        res = await jobpage.goto(loginurl)
        ua = os.environ.get('ua')
        us = os.environ.get('us')

        await jobpage.fill('#num', ua)
        # passwd
        await jobpage.fill('#passwd', us)
        await jobpage.locator('#app > div > div > div:nth-child(3) > div > div > div.card__title.card__title--primary > div:nth-child(3) > button > div').click()
        for url in data.data:
            ...
            data = supabase.table("papers_bianmi").update({"pdfdone":True}).eq("pdflink", url).execute()
            # await asyncio.sleep(3)
            time.sleep(30)

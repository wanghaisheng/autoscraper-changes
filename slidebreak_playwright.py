...
# https://github.com/zhuzikang/douyin_search_user
useragent = [
...
class User_search_douyin():
    ...
    def open_page(self):
        url = "https://www.douyin.com/search/a?source=normal_search&type=user"
...
#   only crop quekou area
    ...
    def user_search_douyin(self, keyword):
        # if random.randint(0,1): # 模拟崩溃
        #     raise Exception()
        # page =self.browser.new_page()

        page = self.page
        page.goto("https://www.douyin.com/search/" + keyword + "?source=normal_search&type=user")
        # try:
        ...
        return self.get_userinfo_douyin(page)

    def get_userinfo_douyin(self, page):  # 提取数据
        ...
        for index in range(1,ul.count()):
            ...
            try:
                ...
                secuid =ul.nth(index).locator('div > a').get_attribute('href').split('?')[0][28:]
